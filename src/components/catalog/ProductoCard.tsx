...
const ProductoCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  const handleAddToCart = () => {
    ...
    addToCart({
      productId: product.id ?? product.Autonumerico ?? product.Codigo_Producto ?? 0,
      name: product.Descripcion || product["Descripción"] || product.nombre || 'Producto',
      price: typeof product.Precio1 === 'number' ? product.Precio1 : (product.precio || 0),
      quantity: cantidad,
    });
    setShowCart(true);
  };
  ...
  return (
    <>
      <div
        key={product.id ?? `${product.codigo}-${product.nombre}`}
        ...
        <div className="relative z-10 flex-grow">
          <ProductImage 
            codigos={[ 
              ...
              product.Personalizado9, 
              product["Personalizado9"], 
              product.CódigoInventario, 
              product["CódigoInventario"], 
              product.CodigoInventario, 
              product["CodigoInventario"]
            ].filter(Boolean)}
            alt={product.Descripcion || product["Descripción"] || 'Producto'} 
          />
          <div className="flex justify-between items-start mb-8">
            ...
            <span className="text-[10px] font-mono text-gray-300 font-bold uppercase">
              #{product.Codigo_Producto || 'ID-XXX'}
            </span>
          </div>
          <h3 className="text-xl font-bold text-[#211F1E] mb-4 leading-[1.1] uppercase italic font-title group-hover:text-[#1A9E53] transition-colors duration-300">
            {product.Descripcion || product["Descripción"] || 'Producto'}
          </h3>
          <div className="mt-6 flex items-baseline gap-2">
            <span className="text-3xl font-black text-[#211F1E]">
              {typeof product.Precio1 === 'number' ? `$${Math.round(product.Precio1).toLocaleString('es-CO')}` : '---'}
            </span>
        ...
        <div className="mt-10 pt-6 border-t border-gray-50 flex items-center justify-between relative z-10">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-14 h-12 rounded-xl border border-gray-200 text-center font-bold text-lg focus:outline-none focus:ring-2 focus:ring-[#1A9E53]"
            />
            <button
              className="h-12 w-12 bg-[#F8F9FA] rounded-2xl flex items-center justify-center text-[#211F1E] group-hover:bg-[#1A9E53] group-hover:text-white transition-all duration-300 shadow-sm"
